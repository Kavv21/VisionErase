...
class TestMetricsImport:
    ...
    def test_metric_count_matches_expected(self):
        """Catches accidental removal of a metric — there must be exactly 28."""
        public = [v for k, v in vars(m).items() if k.isupper() and not k.startswith("_")]
        assert len(public) == 28, f"Expected 28 metrics, found {len(public)}: {[k for k in vars(m) if k.isupper()]}"
